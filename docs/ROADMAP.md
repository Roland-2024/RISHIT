...
Task 01, Tasks 11–13, and supplemental Tasks 05A, 06A, and 08A are merged and `DONE`. **Task 14 — Authoritative totals and fee snapshots** is in `REVIEW`; dependants remain blocked until it is merged. The Task 07 delivery request mapped to Task 11 because the original Task 07 was already merged. External legal/provider evidence work may proceed without changing blocked implementation tasks to `READY`.
...
| 14 | Authoritative totals and fee snapshots | 12, 13 | REVIEW | Shared code calculates signed integer EUR-cent item, approved buyer fee, snapshotted shipping, total, zero seller fees, and seller payable; immutable snapshots retain the policy/version and reject mixed currency, negative input, overflow, and inconsistent totals. | — |
